...
bool Table::CanBuy()
{
    ...
    else if(CurrCard[0] < 3 && CurrCard[0] >= 0 && CurrCard[1] == 4)//为扣了的卡
        return CurrPlayer->CanBuy(CurrPlayer->Reserved[CurrCard[0]]);
    else return false;
}
...
int Table::BuyMani()//买得起的情况下才调用
{
    ...
    {
        CurrPlayer->Buy(CurrPlayer->Reserved[CurrCard[0]]);
