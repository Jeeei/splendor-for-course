...
//    qDebug()<<"operator new: "<<sz<<" Bytes\n";
//    void *m = malloc(sz);
//    if(!m)  qDebug()<<"out of memory";
//    return m;
//}
    ...
    ui(new Ui::gameInterface)
{
    ...
    //选择界面信号连接
    connect(ChooseSurface,SIGNAL(Start()),SLOT(StartSlot()));
    connect(ChooseSurface,SIGNAL(add()),SLOT(AddSlot()));
    connect(ChooseSurface,SIGNAL(subtract()),SLOT(SubtractSlot()));
    connect(ChooseSurface,SIGNAL(image1()),SLOT(Img1Change()));
    connect(ChooseSurface,SIGNAL(image2()),SLOT(Img2Change()));
    connect(ChooseSurface,SIGNAL(image3()),SLOT(Img3Change()));
    connect(ChooseSurface,SIGNAL(image4()),SLOT(Img4Change()));
