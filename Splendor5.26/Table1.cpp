...
Table::~Table()
{
 ...
 void Table::NewSelect()
 {
     for(int i = 0;i < sPlayerNum;i++)
     {
         if(sPlayers[i] == nullptr)
...
bool Table::Start()
{
    ...
    {
        sPlayers[i]->SetImg(GetsPlayerPic(i));//写入头像，名字已经写入
        //qDebug()<<GetPlayerPic(i);
    }
...
QString Table::GetsPlayerPic(int no)
{
    if(no > sPlayerNum || no < 0)
        return "background-color:rgba(255,255,255,80)";
    QString pic = "border-image:url(:images/role/0.jfif)";
    switch (sPlayersPic[no]) {
        case 0:pic = "border-image:url(:images/role/0.jfif)";break;
        case 1:pic = "border-image:url(:images/role/1.jfif)";break;
        case 2:pic = "border-image:url(:images/role/2.jpg)";break;
        case 3:pic = "border-image:url(:images/role/3.jpg)";break;
        case 4:pic = "border-image:url(:images/role/4.jpg)";break;
        case 5:pic = "border-image:url(:images/role/5.jpg)";break;
        case 6:pic = "border-image:url(:images/role/6.jpg)";break;
        case 7:pic = "border-image:url(:images/role/7.jpg)";break;
        case 8:pic = "border-image:url(:images/role/8.jpg)";break;
        case 9:pic = "border-image:url(:images/role/9.jpg)";break;
    default:break;
    }
    return pic;
}
...
{
    if(CurrPlayer->GetReserved(num) != nullptr && num >= 0 && num < 3)
...
QString Table::PlayerImg(int player)
{
    if(player == -1)
        return CurrPlayer->GetAddr();
    else
    {
        for(int i = 0; i < PlayerNum; i++)//寻找下一个玩家
        {
            if(CurrPlayer == Players[i])
            {
                return Players[(i+1)%PlayerNum]->GetAddr();
            }
        }
        return "background-color: rgba(255, 255, 254, 50);border:2px solid rgb(247, 228, 16);border-radius:50px;";
    }
}
...
void Table::GetCurrCard(int *level, int *num)
{
    *level = CurrCard[0];
    *num = CurrCard[1];
}
...
int Table::AvailTakenCardInfo(int level, int num)//加上taken临时判断
{
    if(level>=0 && level<3 && num>=0 && num<4)
    {
        return CurrPlayer->TakeCanBuy(OpenCard[level][num],TakenDiamond);
    }
    else if(level>=0 && level<3 && num == 4)
    {
        return CurrPlayer->TakeCanBuy(CurrPlayer->Reserved[level],TakenDiamond);
    }
    else return false;
}
