    ...
    ui(new Ui::gameInterface)
{
    ...
    {
        ChooseSurface->show();
        ChooseSurface->showImg1(table->GetsPlayerPic(0));
        ChooseSurface->showImg2(table->GetsPlayerPic(1));
    }
}
...
void gameInterface::Next()
{
    ...
    QRect name1 = ui->name1->geometry();
    QRect name2 = ui->name2->geometry();
    ui->name1->setGeometry(name2);
    ui->name2->setGeometry(name1);
    if(table->GetStatus() == Ending)//游戏结束
    {
        ...
        if(num1 == 1)//一个第一
        {
            ...
            end1->showNo1Name(no1->GetName());
            end1->showNo2Name(no2->GetName());
...
void gameInterface::people1_textChanged()
{
    ...
    ui->player1->setStyleSheet("border-radius:60px;"+table->PlayerImg());
    //显示当前玩家拥有的宝石数
...
void gameInterface::people2_textChanged()
{
    ...
    ui->player2->setStyleSheet("border-radius:60px;"+table->PlayerImg(1));
    //显示玩家2拥有的宝石数
